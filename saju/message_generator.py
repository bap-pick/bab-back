...
# 한글 이름만 추출
def extract_hangul_oheng(oheng_key: str) -> str:
    if '(' in oheng_key and ')' in oheng_key:
        return oheng_key.split('(')[1].strip(')')
    return oheng_key # 안전을 위해 괄호가 없으면 그대로 반환

# 오행 유형별 추천 메시지 생성 로직
def get_counter_oheng(oheng: str) -> str:
    oheng = extract_hangul_oheng(oheng) # 헬퍼 함수 사용
...
def define_oheng_messages(lacking: List[str], strong: List[str], oheng_type: str) -> Tuple[str, str]:
    ...
    if oheng_type == "균형형":
        ...
        lacking_hangul = extract_hangul_oheng(lacking[0])
        analysis_headline = f"{lacking_hangul[0]} 기운이 거의 없는 무형"
    else: # 치우침형
        lacking_hangul = extract_hangul_oheng(lacking[0])
        strong_hangul = extract_hangul_oheng(strong[0])
        analysis_headline = f"{lacking_hangul[0]} 기운이 부족하고 {strong_hangul[0]} 기운이 강해요!"
    ...
    if oheng_type == "균형형":
        ...
        lacking_name = extract_hangul_oheng(lacking[0])
        lacking_name2 = extract_hangul_oheng(lacking[1])
        ...
        # 2순위: 강한 오행 제어 조언
        if strong:
            strong_name = extract_hangul_oheng(strong[0])
